...
class ShpFileHandler:
    ...
    """
    def __init__(self, year: int, **geographic_specifiers: object) -> None:
        self._year = year
        self._geographic_specifiers = geographic_specifiers

        # Stash the scope we wish to query TIGER shpfile geometric info against.
        self._for_scope, self._scope, self._folder = self._tiger_scope(**geographic_specifiers)

        # Stash the outer reference point (to check if it requires state)
        self._outer = self._needs_scope(year, **geographic_specifiers)

    
    def _tiger_scope(self, **geographic_specifiers):
        """
        ...
        match shp_scope:
            case 'CD':
                shp_scope += str(_congressional_district_rule(self._year))
            case 'ZCTA':
                shp_scope, folder = _zipcode_tabulation_area_rule(self._year)
            case 'PUMA':
                shp_scope, folder = _public_use_microdata_area_rule(self._year)
            case 'SUBMCD':
                shp_scope, folder = _subminor_civil_division_rule(self._year)
            case 'UAC':
                shp_scope, folder = _urban_area_rule(self._year)
        
        return for_scope, shp_scope, folder
    
    def _needs_scope(self, year: int, **geographic_specifiers):
        """
    ...
    def _tiger_url_fmtter_2008_2009(self):
        base_url = f'https://www2.census.gov/geo/tiger/TIGER{self._year}/'
        ...
        if self._scope in ['BG', 'COUSUB', 'PLACE', 'TRACT', 'SLDL', 'SLDU', 'UNSD'] \
            ...
            # Hmm...
            path = f'tl_{self._year}_us_{self._scope.lower()}.zip'

        return base_url, path


    def _tiger_url_fmtter_pre_2010_and_2010(self):
        base_url = 'https://www2.census.gov/geo/tiger/TIGER2010/{self._folder}/{nested}/'

        outer = 'us'
        if self._outer == 'state':
            outer = self._geographic_specifiers.get('state', '')

        nested = self._year
        if self._scope.startswith( ('CD',) ):
            nested = drop_alphaletters_re.sub('', self._scope)

        suffix = str(self._year)[-2:]
        if self._scope.startswith( ('CD', 'PUMA', 'ZCTA', 'UAC') ):
            suffix = ''
        
        base_url = f'https://www2.census.gov/geo/tiger/TIGER2010/{self._folder}/{nested}/'
        path = f'tl_2010_{outer}_{self._scope.lower()}{suffix}.zip'

        return base_url, path


    def _tiger_url_fmtter_post_2010(self):
        outer = 'us'
        if self._outer == 'state':
        ...
        base_url = f'https://www2.census.gov/geo/tiger/TIGER{self._year}/{self._folder}/'
        path = f'tl_{self._year}_{outer}_{self._scope.lower()}.zip'

        return base_url, path

    def _tiger_url_fmtter(self) -> t.Tuple[str, str]:
        if 2008 <= self._year <= 2009:
            return self._tiger_url_fmtter_2008_2009()
        elif self._year == 2010 or self._year < 2008:
    ...
    def fetch_tiger_shpfile(self) -> t.Optional[gpd.GeoDataFrame]:
        ...
        **NOTE**: We try our best to fit as many geometries as possible. However,
        ...
        Returns
        -------
        If the appropriate TIGER shapefile is found, returns a
        :py:class:`geopandas.GeoDataFrame` instance.
        """
    ...
    def _fetch_tiger_shpfile(self) -> t.Optional[gpd.GeoDataFrame]:
        """
        ...
        """
        base_url, path = self._tiger_url_fmtter()
        ...
        try:
            ...
            msg = \
            f"\nCould not extract the appropriate TIGER shapefile for the '{self._for_scope}' scope \n" \
            f"during the {self._year} calendar year. This is partially due to several reasons: \n" \
    ...
    def _tiger_shpfile_fmtter(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
        ...
        id_cols = GEO_SPEC_METADATA[self._for_scope][3]
